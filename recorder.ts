...
export class Frequencies {
  ...
  constructor(
    readonly frequency_scaler: number,
    readonly complex_data: number[],
  ) {
    this.amplitudes = new Array(complex_data.length/2)
    for (let index = 0; index < this.amplitudes.length; ++index) {
      this.amplitudes[index] = Math.sqrt(complex_data[index*2]**2 + complex_data[index*2+1]**2);
    }
  }
}
...
export function start(
  ...
  on_frequencies: (detailed_frequencies: Frequencies, responsive_frequencies: Frequencies) => void,
): void {
  ...
  recording_process.stdout.on('data', (samples: Buffer) => {
    ...
    let current_ring_buffer_index = ring_buffer_index-1;
    ...
    on_frequencies(
      new Frequencies(
        EVENTS_PER_SECOND/BUFFERED_SAMPLES,
        fft_detailed_output.slice(0, 2 * OUTPUT_FREQUENCIES * BUFFERED_SAMPLES)
      ),
      new Frequencies(
        EVENTS_PER_SECOND/RESPONSIVE_SAMPLES,
